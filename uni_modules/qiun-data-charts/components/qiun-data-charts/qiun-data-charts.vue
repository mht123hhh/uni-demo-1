...
<script>
    ...
    for (let i in args) {
      for (let key in args[i]) {
        if (args[i].hasOwnProperty(key)) {
          origin[key] = args[i][key] && typeof args[i][key] === 'object' ? deepCloneAssign(Array.isArray(args[i][key]) ?
  ...
  export default {
    ...
    methods: {
      beforeInit() {
        this.mixinDatacomErrorMessage = null;
        if (typeof this.chartData === 'object' && this.chartData != null && this.chartData.series !== undefined && this
        ...
        if (tc && tc !== undefined && tc !== null) {
          ...
          cfu.instance[cid].showToolTip(e, {
            ...
            formatter: (item, category, index, opts) => {
              if (typeof cfu.option[cid].tooltipFormat === 'string' && cfu.formatter[cfu.option[cid]
                  .tooltipFormat]) {
      ...
      _touchMove(e) {
        ...
        if (this.ontouch === true && cfu.option[cid].enableScroll === true && this.onzoom === true && e.changedTouches
          .length == 2) {
...
<script module="rdcharts" lang="renderjs">
  ...
  function rddeepCloneAssign(origin = {}, ...args) {
    for (let i in args) {
      for (let key in args[i]) {
        if (args[i].hasOwnProperty(key)) {
          origin[key] = args[i][key] && typeof args[i][key] === 'object' ? rddeepCloneAssign(Array.isArray(args[i][
            key
          ]) ? [] : {}, origin[key], args[i][key]) : args[i][key];
        }
      }
  ...
  export default {
    ...
    methods: {
      ...
      updataEChart(cid, option) {
        ...
        if (option.tooltip) {
          ...
          if (typeof option.tooltipFormat === 'string' && cfe.formatter[option.tooltipFormat]) {
            option.tooltip.formatter = option.tooltip.formatter ? option.tooltip.formatter : cfe.formatter[option
              .tooltipFormat]
          }
        }
        // 颜色渐变添加的方法
      ...
      touchMove(e) {
        ...
        if (cfu.option[cid].ontap === true && cfu.option[cid].enableScroll === false && cfu.option[cid].onmovetip ===
          true) {
          let rchartdom = document.getElementById('UC' + cid).getBoundingClientRect()
          let tmpe = {
